...
{
    public class SpritePhysicsOutlineProvider: ArtDataProvider, ISpritePhysicsOutlineDataProvider
    {
        ...
        public void SetOutlines(GUID guid, List<Vector2[]> data)
        {
            ...
            if(sprite != null)
            {
                ((ArtSpriteMetaData)sprite).spritePhysicsOutline = data.Select(x => new ArtSpriteOutline() { outline = x }).ToList();
            }
        }
