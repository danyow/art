...
namespace UnityEditor.U2D.ART
{
    ...
    internal class ImagePackNode
    {
        ...
        {
            visitor.Visit(this);
            if(left != null) left.AcceptVisitor(visitor);
            if(right != null) right.AcceptVisitor(visitor);
        }
        ...
        public bool TryInsert(ImagePacker.ImagePackRect insert, int padding, out Vector2Int remainingSpace)
        {
            ...
            if(insertWidth > rect.width || insertHeight > rect.height) return false;
            ...
            {
                Vector2Int spaceLeft,
                           spaceRight;
                bool insertLeft,
                     insertRight;
                ImagePackNode tryLeft,
                              tryRight;
                tryLeft = left;
                tryRight = right;
                ...
                else if(insertLeft) remainingSpace = spaceLeft;
                else if(insertRight) remainingSpace = spaceRight;
                else return false;
            }
        ...
        static bool SplitRects(ImagePackNode node, ImagePacker.ImagePackRect insert, int padding, out ImagePackNode left, out ImagePackNode right)
        {
            ...
            left = right = null;
            var tryRects = new[] { new ImagePackNode(), new ImagePackNode(), new ImagePackNode(), new ImagePackNode() };
        ...
        public bool Insert(ImagePacker.ImagePackRect insert, int padding)
        {
            ...
            else
            {
                ...
                if(insertLeft && insertRight)
                {
                    if(spaceLeft.sqrMagnitude < spaceRight.sqrMagnitude) left.Insert(insert, padding);
                    else right.Insert(insert, padding);
                }
                else if(insertLeft) left.Insert(insert, padding);
                else if(insertRight) right.Insert(insert, padding);
                else return false;
            }
            return true;
        }
